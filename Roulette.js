...
let DataStore = { "wins": 0, "loses": 0, "most": 0, "least": 0, "spinAgainCount": 0 };
...
function startStorage() {
    ...
    if (LS.getItem("NUM_WINS") !== null) {
        ...
        DS.spinAgainCount = parseInt(LS.getItem("SPIN_AGAIN_COUNT")) || 0; // Add this line
    }
    //CASE -- data needs established --> initialize values in LS
        ...
        LS.setItem("SPIN_AGAIN_COUNT", DS.spinAgainCount); // Add this line
    }
    return DS;
}
...
function refreshStats() {
    ...
    $("#spin-again-count").text(DS.spinAgainCount); // Add this line
}
//resets our games stats in localStorage
function resetLocalStorage() {
    const conf = confirm("Are you sure you want to reset you game stats?");
    if (conf) {
        DataStore = { "wins": 0, "loses": 0, "most": 0, "least": 0, "spinAgainCount": 0 }; // Add spinAgainCount: 0
        refreshStats();
    }
}
...
$("#reset-btn").click(() => {
    // Increment spin again counter
    DataStore.spinAgainCount += 1;
    LocalStore.setItem("SPIN_AGAIN_COUNT", DataStore.spinAgainCount);

    // Refresh stats to show updated count
    refreshStats();

    // Reload the page after short delay so stats update first
